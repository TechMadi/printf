...
.B s
.in +2n
OBOBOBThe \fIconst char *\fR argument is expected to be a pointer to a character
OBOBOBarray (pointer to a string). Characters from the array are written starting
from the first element of the array and ending at, but not including, the
OBOBOBterminating null byte ('\\0').
.in
.RE
OBOBOB
.RS
...
OBOBOBarray are written in reverse, starting from (but not including) the
